...
export default async function genImageData() {
  ...
  const collection = currentPage[6] != 'pages' ? currentPage[6]?.concat('/') : null;
  const ref =
    currentPage.includes('entries') && collection != null
      ? 'cms/' + collection + currentPage[8]
      : null;

  const newImagePath = await getNewImagePath(ref);

  const assetContentResponse = await octokit.request(
    'GET /repos/usdoj-crt/beta-ada/contents/assets/',
    {
      owner: 'usdoj-crt',
      repo: 'beta-ada',
      path: 'assets/',
  ...
  const imageFolderSha = assetContentResponse.data
    .filter((folder) => folder['name'] === 'images')
    .map((folder) => folder['sha'])[0];
  const treeResponse = await octokit.request(
    'GET /repos/usdoj-crt/beta-ada/git/trees/' + imageFolderSha + '?recursive=1',
    {
      owner: 'usdoj-crt',
      repo: 'beta-ada',
...
async function getNewImagePath(ref) {
  ...
  const PR = await octokit.request(
    'GET /repos/usdoj-crt/beta-ada/commits/' + commitSha.data + '/pulls',
    {
      owner: 'usdoj-crt',
      repo: 'beta-ada',
  ...
  const prFiles = await octokit.request(
    'GET /repos/usdoj-crt/beta-ada/pulls/' + prNumber + '/files',
    {
      owner: 'usdoj-crt',
      repo: 'beta-ada',
  ...
  const fileNames = prFiles.data
    .filter((prFile) => assetFileTypes.indexOf(prFile.filename.split('.')[1]) !== -1)
    .map((prFile) => {
